...
<script type="text/javascript">{literal}
    ...
    <form action="/ProductAdmin/Edit/{$product->id}" method="post" enctype="multipart/form-data">
        ...
        <div id="general" class="tab" {if !$smarty.post.active_tab || $smarty.post.active_tab=='general'}style="display: block;"{/if}>
            ...
            <label>Name:</label>
            <br />
            <input class="title" type="text" name="name" value="{$product->name}" maxlength="255" />

            <br />
            <label>UPC:</label>
            <br />
            <div type="quiet">Unique product code</div>
            <input class="text" type="text" name="upc" value="{$product->upc}" maxlength="255" />
...
*}
            ...
            <br />
            <input class="text" type="text" name="keywords" value="{$product->keywords}" maxlength="255" />

            <br />
            <label>Condition:</label>
            <br />
            <input class="text" type="text" name="condition" value="{$product->condition}" maxlength="255" />
        ...
        <div id="data" class="tab"{if $smarty.post.active_tab=='data'} style="display: block;"{/if}>
            ...
            <br />
            <input class="text" type="text" name="price" value="{$product->price}" maxlength="255" />
            ...
            <br />
            <input class="text" type="text" name="quantity" value="{if $product->variants}See 'Variants' tab" disabled="disabled{else}{$product->quantity}{/if}" maxlength="255" />

            <br />
            <label>Weight:</label>
            <br />
            <div type="hint">In grams</div>
            <input class="text" type="text" name="weight" value="{$product->weight}" maxlength="255" />

            <br />
            <label>Storage location:</label>
            <br />
            <input class="text" type="text" name="storage_location" value="{$product->storage_location}" maxlength="255" />
        ...
        <div id="images" class="tab"{if $smarty.post.active_tab=='images'} style="display: block;"{/if}>
            ...
            {foreach from=$product->images item=image}
                <div style="border: 1px dashed #434343; margin: 10px; padding: 10px; width: 150px; float: left;">
                    ...
                    <br />
                    <input class="text" type="text" name="image_title_{$image->id}" value="{$image->title}" maxlength="255" style="width: 140px;" />
            ...
            <table>
                ...
                {foreach from=$product->variants item=variant}
                    <tr>
                        ...
                        <td>{$variant->name}</td>
                        <td>
                            <input type="text" name="variant_price_change_{$variant->id}" value="{$variant->price_change}" style="width: 50px;" />%
                        </td>
                        <td>
                            <input type="text" name="variant_quantity_{$variant->id}" value="{$variant->quantity}" style="width: 50px;" />
                        </td>
                    </tr>
                {/foreach}
                </tboody>
                <tfooter>
                    <tr>
                        <td>New</td>
                        <td>
                            <input type="text" name="new_variant_type" style="width: 100px;" value="" />
                        </td>
                        <td>
                            <input type="text" name="new_variant_name" style="width: 150px;" value="" />
                        </td>
                        <td>
                            <input type="text" name="new_variant_price_change" style="width: 50px;" value="" />
                        </td>
                        <td>
                            <input type="text" name="new_variant_quantity" style="width: 50px;" value="" />
                        </td>
                    </tr>
                </tfooter>
            </table>
